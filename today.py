"""
Self-updating GitHub profile README script for knokvik (Niraj Naphade).
Based on the architecture popularized by Andrew6rant.

Runs daily via GitHub Actions:
  - Computes "Uptime" (age) from BIRTHDAY
  - Queries GitHub GraphQL for repos, stars, commits, followers, LOC
  - Writes live stats into dark_mode.svg and light_mode.svg
"""
...
# ---------------------------------------------------------------------------
# Birthday used for the "Uptime" field on the card
BIRTHDAY = datetime.datetime(2006, 7, 30)  # TODO: set your real birthday (YYYY, M, D)

# Cache file header size (lines reserved as comments at top of cache/*.txt)
CACHE_COMMENT_SIZE = 7

# Fine-grained personal access token with All Repositories access:
# Account permissions: read:Followers, read:Starring, read:Watching
# Repository permissions: read:Contents, read:Metadata
# (Commit statuses / Issues / PRs optional for future use)
HEADERS = {"authorization": "token " + os.environ["ACCESS_TOKEN"]}
USER_NAME = os.environ["USER_NAME"]  # 'knokvik'
QUERY_COUNT = {
    "user_getter": 0,
    "follower_getter": 0,
    "graph_repos_stars": 0,
...
def recursive_loc(
    ...
    raise Exception(
        "recursive_loc() has failed with a", request.status_code, request.text, QUERY_COUNT
    )
...
def svg_overwrite(
    filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data
):
...
if __name__ == "__main__":
    """
    knokvik (Niraj Naphade) — self-updating profile card
    """
    ...
    total_loc, loc_time = perf_counter(
        loc_query, ["OWNER", "COLLABORATOR", "ORGANIZATION_MEMBER"], CACHE_COMMENT_SIZE
    )
    if total_loc[-1]:
