<!-- knokvik profile README -->

<p align="left">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="dark_mode.svg?v=632a6d17">
    <img alt="Niraj Naphade (knokvik)" src="light_mode.svg?v=632a6d17" width="985">
  </picture>
</p>

### Featured work

| Project | Stack | What it is |
| --- | --- | --- |
| [kestral](https://github.com/knokvik/kestral) | C++ | High-throughput document processing engine |
| [market-making-engine](https://github.com/knokvik/market-making-engine) | TypeScript | LOB market-maker with Avellaneda–Stoikov quoting |
| [visionkv](https://github.com/knokvik/visionkv) | Python | Modality-aware KV-cache research for VLMs |
| [TypeRush](https://github.com/knokvik/TypeRush) | TypeScript | Real-time typing battles (Next.js + Socket.io) |
| [redteamlab](https://github.com/knokvik/redteamlab) | Python | AI-assisted cybersecurity testing / AIVSS |
| [ocula](https://github.com/knokvik/ocula) | TypeScript | Stepwise ML model visualiser |

<p align="center">
  <a href="https://knokvik.vercel.app/">Portfolio</a> ·
  <a href="https://www.linkedin.com/in/niraj-naphade/">LinkedIn</a> ·
  <a href="https://github.com/knokvik">GitHub</a>
</p>
